...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Select(shows => shows.Name).OrderBy(s => s).ToList();
        }
        ...
        {
            return shows.Select(shows => shows.EndYear).Max();
        }
        ...
        {
            return shows.Where(s => s.ImdbRating > shows.Select(s => s.ImdbRating).Min()).ToList();
        }
        ...
        {
            return shows.OrderByDescending(s => s.ImdbRating).Where(shows => shows.Genres.Contains("Comedy")).Select(shows => shows.Name).ToList();
        }
        ...
        {
            List<Show> MaxEps = shows.OrderByDescending(s => s.EpisodeCount).ToList();
            return shows.FirstOrDefault(s => s.EpisodeCount == MaxEps[0].EpisodeCount);
        }
        ...
        {
            List<Show> OrderedByEndYear = shows.OrderBy(s => s.EndYear).ToList();
            return OrderedByEndYear.FirstOrDefault(s => s.EndYear >= 2000);
        }
        ...
        {
            List<Show> LessThan8 = shows.OrderByDescending(s => s.Name).Where(s => s.ImdbRating < 8).ToList();
            return LessThan8.FirstOrDefault(s => s.EndYear - s.StartYear > 10);
        }
